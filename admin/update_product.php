...
if (isset($_POST['update'])) {
    ...
    $sql = "UPDATE `products` SET name = '$name', price = '$price', details = '$details' WHERE id = '$pid'";
    ...
    if (!empty($image_01)) {
        ...
        if ($image_size_01 > 2000000) {
            ...
            if (mysqli_query($conn, $update_image_01)) {
                move_uploaded_file($image_tmp_name_01, $image_folder_01);
                unlink('../uploaded_img/' . $_POST['old_image_01']);
                $message[] = 'Image 01 updated successfully!';
            } else {
                $message[] = 'Failed to update image 01: ' . mysqli_error($conn);
            }
        }
    }

    // Handle image updates (image 02)
    ...
    if (!empty($image_02)) {
        ...
        if ($image_size_02 > 2000000) {
            ...
            if (mysqli_query($conn, $update_image_02)) {
                move_uploaded_file($image_tmp_name_02, $image_folder_02);
                unlink('../uploaded_img/' . $_POST['old_image_02']);
                $message[] = 'Image 02 updated successfully!';
            } else {
                $message[] = 'Failed to update image 02: ' . mysqli_error($conn);
            }
        }
    }

    // Handle image updates (image 03)
    ...
    if (!empty($image_03)) {
        ...
        if ($image_size_03 > 2000000) {
            ...
            if (mysqli_query($conn, $update_image_03)) {
                move_uploaded_file($image_tmp_name_03, $image_folder_03);
                unlink('../uploaded_img/' . $_POST['old_image_03']);
                $message[] = 'Image 03 updated successfully!';
            } else {
                $message[] = 'Failed to update image 03: ' . mysqli_error($conn);
            }
        }
    }
}
